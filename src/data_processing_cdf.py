...
PROCESSED_DATA_FILE = os.path.join(PROCESSED_DATA_DIR, 'processed_data_cdf.npz')  # 儲存處理後數據的檔案名稱和路徑
...
def generate_ground_truth(label, gesture_type, total_classes, distribution='pdf'):
    """
    ...
    Args:
        ...
        distribution (str): 指定生成 'pdf' 還是 'cdf' 分佈
    Returns:
    ...
    sigma = length / 6
    gaussian_curve = np.exp(-0.5 * ((x - length // 2) / sigma) ** 2)

    if distribution == 'cdf':
        # 生成 CDF 曲線
        cdf_curve = np.cumsum(gaussian_curve)
        cdf_curve /= cdf_curve[-1]  # 正規化，使其最大值為 1
        ground_truth[start_idx:end_idx+1, gesture_type] = cdf_curve
    else:
        # 默認生成 PDF 曲線
        gaussian_curve /= gaussian_curve.max()  # 正規化，使其最大值為 1
    ...
    for gesture_idx, gesture_type in enumerate(gesture_types):
        ...
        for file_name in os.listdir(gesture_dir):
            if file_name.endswith('.h5'):
                ...
                # 生成 ground truth
                ground_truth = generate_ground_truth(label, gesture_idx, len(gesture_types), distribution='cdf')
